...
//If set to true, Japan prices will be displayed
var showJPPrice = GM_getValue("showJPPrice", true);
...
function SteamPage(countryName, countryCode, currencyCode, show, valuepattern, replaceVal, vat, globalCountryName, dependedPage) {
    ...
    this.findPrice = function() {
        ...
        try {
            ...
            this.price = parseFloat(this.valuepattern.exec(this.priceHtml)[1].replace(replaceVal[0], replaceVal[1]).replace(",", "."));
            if (this.vat > 0) {
...
var jpvaluepattern = new RegExp(/&#165; ([\d,]+)/i);
...
var eu2 = new SteamPage('EU Tier 2', tier2cc, 'EUR', showTieredEuPrices, euvaluepattern, ["--", "00"], 0);
var eu1 = new SteamPage('EU Tier 1', tier1cc, 'EUR', showTieredEuPrices, euvaluepattern, ["--", "00"], 0, 'EU', eu2);
...
var jp  = new SteamPage('JP',       'jp', 'JPY', showJPPrice, jpvaluepattern, [",", ""], 0);
...
if (yourBaseCountry != null)
{
    for (var j = 0; j < pages.length; j++)
        if (pages[j].countryName== yourBaseCountry)
        {
            baseCountry = pages[j];
            baseCountry.show = true;
            baseCurrency = baseCountry.currencyCode;
            break;
        }
}
...
function stateChanged() {
    ...
    var calcscript = "function getDifference(currency, basePrice, localPrice) " +
        ...
        "  if (currency == 'GBP') {convertedToLocal = " + baseCurrency + "toGBP(basePrice);}\n" +
        "  else if (currency == 'EUR') {convertedToLocal = " + baseCurrency + "toEUR(basePrice);}\n" +
        "  else if (currency == 'RUB') {convertedToLocal = " + baseCurrency + "toRUB(basePrice);}\n" +
        "  else if (currency == 'BRL') {convertedToLocal = " + baseCurrency + "toBRL(basePrice);}\n" +
        "  else if (currency == 'JPY') {convertedToLocal = " + baseCurrency + "toJPY(basePrice);}\n" +
        "  else if (currency == 'USD') {convertedToLocal = " + baseCurrency + "toUSD(basePrice);}\n" +
